...
% % \title{Relexation of functional-consistency allows
% % a non-contextual hidden variable model for quantum mechanics
% %\\
% % \sout{A non-contextual Hidden Variable Model for Quantum
% % Mechanics}}
% \shorttitle{Admissibility of non-contextual HV models in QM} 
% %Non-contextual HV Model for QM} %Insert here a short version of the title if it exceeds 70 characters
...
%   \inst{2} Centre for Quantum Information and Communication, Ecole polytechnique de Bruxelles, CP 165,\\Universit\'e libre de Bruxelles (ULB), 1050 Brussels, Belgium \\
...
%%%%%%%%%%%%%%%%%%Injected from Arvind sir's PLA draft variant%%%%%%%%%%%%%%%
\usepackage{paralist}
\usepackage{hyperref}
%\makeatother
\usepackage[english]{babel}
...
hidden variable model consistent with all { the}
...
with QM. { In our construction,} we define a
weaker variant of this restriction which captures its
essence while still allowing a non-contextual description of
QM. This is in contrast to { the} contextual
 hidden variable {toy} models, such as
the one by Bell, { and}  brings out an interesting
alternate way of looking at QM. { The results}
also relate to { the Bohmian}
model, where it is harder to pin
down such features.\end{abstract}
%\begin{document}
...
and
cryptography~\cite{HowardCntxCmptn,CabelloCntxScrt,Raussendorf2013,Singh2017}.
...
which additionally satisfy \emph{functional-consistency---algebraic 
constraints obeyed by commuting quantum
...
provides a map, for all observables in $\hat{\mathcal{H}}$, $M: \{
\hat{\mathcal{H}},[\mathcal{H}] \} \to\mathbb{R}$
...
aspect of QM is defined to be a context which in turn can potentially be used to demonstrate contextuality. 
...
\begin{defn}
A prediction map $M$ is \emph{functionally-consistent} if
\begin{align*} &
...
\begin{proof} Due to Peres Mermin for
...
assign a value to an arbitrary observable, given in terms of its spectral decomposition as,


$ \hat{A}=\sum_{a\in a_1,a_2,\dots a_n }a\left|a\right\rangle
\left\langle a\right| $ with $a_1\le a_2 \le \dots a_n$. \sout{which has eigenvectors $\{
...
$\hat{A}$ is given by finding the smallest $j$ 
s.t.  $c\le\sum_{a'=a_1}^{a_j}\left|\left\langle
...
$S$.
\end{enumerate}
The above HV model is expected to work for all quantum systems. 
We bolster this claim by applying it to a situation where it is 
most likely to fail, i.e. the aforesaid Peres Mermin situation.
\sout{The above HV model is expected to work for all quantum systems,
however, to illustrate its working,} First,
...
$\hat{C}_{3}$. \sout{\color{blue} Atul: Kishor could you please verify the calculation.} To simplify calculations, we note
\[\left|00\right\rangle
=\frac{1}{\sqrt 2} \left[ \left(\left| \uparrow \downarrow \right\rangle
+\left|\downarrow \uparrow \right\rangle
\right)/\sqrt{2}+\left(\left|\uparrow \uparrow \right\rangle
+\left|\downarrow \downarrow \right\rangle
\right)/\sqrt{2}\right] , 
\] with $\ket{\uparrow} = (\ket{0}+i\ket{1})/\sqrt{2}$ and $\ket{\downarrow}=(\ket{0}-i\ket{1})/\sqrt{2}$
...
% \label{eq:toyModel}
\end{equation*}
\caption{For a given initial state, we apply our HV model to the Peres Mermin square (see Equation \ref{eqn:PM}; $R_j$ ($C_j$) refers to product of the observables along the $j^{\text{th}}$ row (column)) and explicitly write the predictions. At each step encoded by $i$, a value of $c\in[0,1]$ is picked from a uniformly random distribution. This, together with the state, completely specifies the predicted value of each observable which we denote by the map $M_1$. An observable is then selected and measured, which is specified in the second last row  (together with the predicted value assigned to it by the HV model) and the state is then updated to the one specified in the last row using the rules specified by the HV model. This state becomes the initial state for the next step. Note that we are able to assign values to the Peres Mermin square and they exhibit non functional-consistency. To see this, recall that the product of the numbers along any given row should be one if the assignment satisfies functional-consistency but this is not the case even in the very first row of $M_1(\hat A _{ij})$.}
\label{tbl:HVmodel}
\end{table*}
{\sloppy
...
construction which is non-contextual (and only has \emph{weak
...
\left\langle 11\right|\right), \end{split} \\ 
\label{eqn:B2} \begin{split}
\hat{B}_{2}\!=\!\hat{\mathbb{I}}\otimes\hat{\sigma}_{z} =& \left|00\right\rangle \left\langle 00\right|+\left|10\right\rangle \left\langle 10\right| \\ 
&-\left(\left|01\right\rangle \left\langle 01\right|+
\left|11\right\rangle \left\langle 11\right|\right), \end{split}\\
\label{eqn:C} \begin{split}
\hat{C} =& f(\{\hat{B}_{i}\})=\\
& 1.\left|00\right\rangle \left\langle
...
11\right|. \end{split}
\end{eqnarray}
We introduced $\hat{C}$ in Equation \ref{eqn:C} as an example of a \emph{refinement}, i.e. an observable 
whose measurement can reveal the values of other compatible observables
which in this case are $\hat B_1$ and $\hat B_2$ as defined in Equations \ref{eqn:B1} and \ref{eqn:B2}. To see this, note that $\hat C$
maybe viewed as a function of
...
of $\hat{B}_{2}$ depends on whether it is measured
alone or with the remaining $\{\hat{B}_{i}\}$.
However, it must be noted that $\hat{B}_{2}$ has a
...
more basic. The advantages in collaborative tasks such as quantum key distribution \cite{Ekert} and randomness certification \cite{colbeck2009quantum}, to name a few, can be interpreted as physical manifestation of non functional-consistency in quantum mechanics.
\sout{\color{blue} Atul: The referee wants a physical meaning of this. I am not sure how to respond.}

\section{Conclusion}
 \sout{\color{blue} Atul: The refere wants this section to be made more particular; feel free to improve upon/add to the following.}
In this letter we have presented
\emph{non functional-consistency} as an alternative to
contextuality \sout{and as an essential signature of
quantumness at the kinematic level} which bolsters the stance that one can not, and must not,
deduce from the KS theorem that quantum mechanics is contextual without further investigation. We 
also showed how viewing quantum mechanics this way already clarifies various aspects of Bohmian Mechanics, a completion of quantum mechanics whose proponents included Bell himself. Our result
points to a (quantum) dynamical exploration of contextuality which 
so far has effectively been studied kinematically only. This should
help us further rule out specific kinds of contextual/non functionally-consistent models and 
clarify what quantum mechanics can not be thought of as.
...
In addition to its intrinsic value to the fundamental understanding of quantum mechanics, we expect \sout{our result} this investigation to also provide new insights that will be useful
in the area of quantum computation. In particular, it has been shown that an established measure of contextuality can be related directly to a measure of the quantum advantage obtained in the measurement based model of quantum computing \cite{Raussendorf2013} as well as to that in the fault-tolerent model \cite{HowardCntxCmptn}. A dynamical study of contextuality/non functional-consistency is expected to open up similar connections and deepen our understanding of the strange computational power of quantum mechanics.



\section{Acknowledgments}
{
  ASA and KB acknowledge funding from KVPY, DST, India. Arvind also acknowledges the DST, India, for support through Grant No.
  EMR/2014/000297. ASA acknowledges the Belgian Fonds de la Recherche Scientifique (FNRS) for support through 
  grants no F.4515.16 (QUICTIME), R.50.05.18.F (QuantAlgo) and F3/5/5MCF/XH/ FC16749 (FRIA). KB acknowledges the CQT Graduate Scholarship.
}
