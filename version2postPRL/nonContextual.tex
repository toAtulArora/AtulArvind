...
\title{Non-contextual Hidden Variable Model for Quantum Mechanics}
\shorttitle{Non-contextual HV Model for QM} %Insert here a short version of the title if it exceeds 70 characters

\author{Atul Singh Arora\inst{1,2} \and Arvind\inst{1} \and Kishor Bharti\inst{3}}
...
\institute{                    
  ...
  \inst{2} Université libre de Bruxelles, \\50 av. F. D. Roosevelt - CP 165/59, B-1050 Brussels, Belgium.\\
...
\abstract{ \color{blue}
We construct a simple non-contextual hidden variable model consistent with all kinematic predictions of quantum mechanics (QM). The famous Bell-KS theorem shows that non-contextual models which satisfy a further reasonable restriction are inconsistent with QM. We define a weaker variant of this restriction which captures its essence while still allowing a non-contextual description of QM. This is in contrast to contextual toy hidden variable models, such as the one by Bell, and brings out an interesting alternate way of looking at QM. This is also related to Bohm's model where it is harder to pin down such features.}
...
{\color{blue} While there have been generalisations, in this letter, we restrict ourselves to the standard notion of non-contextuality as used by KS [??].} 
...
{\color{blue}
The KS theorem is applicable to non-contextual HV models which additionally satisfy functional consistency --- algebraic constraints obeyed by commuting quantum observables must be satisfied by the HV model at the level of individual outcomes. One of the main justifications given for imposing this was the reasonable requirement that an observable $\hat A^2$ must depend on $\hat A$ even at the HV level because otherwise one can construct HV models where these observables get mapped to random variables with no relation to each other (see [??]). As we will show, one can weaken this requirement into what we call weak functional consistency --- functional consistency is demanded only when the observables in question have sharp values. This entails that even in the HV model the observables will depend on each other where they should thereby capturing the same idea. The consequence of demanding only the weaker variant is that the KS theorem no longer applies and non-contextual HV models can be consistently constructed. This is in stark contrast with contextual HV models, such as the one by Bell, which we demonstrate by re-examining a `proof of contextuality' using our model. 
}
\sout{
...
We introduce some notation and make the relevant notions precise to facilitate the description and assesment of our model.
\begin{notn*}
...
\begin{defnrem}
{\color{blue} Broader definitions in the literature have been suggested~\cite{spekkens_pra} which extend the notion to probabilistic HV models. The idea is that any feature of a HV model that is not determined solely by the operational aspect of QM is defined to be, in the case of measurements for instance, measurement contextuality. To maintain a distinction between different features of HV models, which is of interest here, we stick to the standard definition.}
\sout{
...
{\color{blue} 
In addition to a HV model being non-contextual Kochen and Specker~\cite{KochenSpecker} demand the following to establish their no-go theorem.
}
...
$[\psi]\in[\mathcal{H}]$. A
\emph{functionally inconsistent} map is one that is not
...
$\hat{C}$. {\color{blue} In our treatment, instead of imposing functional consistency, we demand its aforesaid weaker variant. It captures the same idea, however, only when it has a precise meaning according to QM.} To capture this property precisely we define
...
{ \color{blue} Before proceeding to do so explicitly we observe that \emph{weak functional consistency} is, in fact, a consequnce of QM.}
\begin{prop*} 
...
{\color{blue} To see this functional inconsistency explicitly in} our model and to see its
...
\label{tbl:HVmodel}
\end{table*}
The \emph{functional inconsistency} is
...
{ \color{blue} 
The model demonstrates \emph{function inconsistency} as an alternative signature of quantumness as opposed to contextuality. This view is not just an artifact of the simplicity of the model. 
...
in BM, does not inolve  overlapping sets of
...
outcomes turns out to be \emph{functionally inconsistent},
suggesting that \emph{functional inconsistency} is a more
suitable explanation of non-classicality
of BHVM.
{\color{blue} In fact, our model when appropriately extended to continous variables yields Bohmian trajectories in the single particle  case which suggests that it can be used as a starting point for constructing more interesting families of HV theories that take quantum time dynamics into account as well.

Bell himself had constructed a deceptively similar toy model to demonstrate a HV construction that is not ruled out by his contextuality no-go theorem. However, his model was contextual (and functionally consistent) which is in stark contrast with our construction which is non-contextual (and functionally inconsistent).

We end with making two short remarks.} First, we illustrate how functional inconsistency gives rise
to situations which could get confused with the
presence of contextuality.  Imagine for two spin
1/2 particles
\begin{eqnarray*}
...
viz. the theory is functionally inconsistent. Note that
...
{\color{blue} Second, note that} \sout{Clearly,} entanglement is not necessary to demonstrate
functional inconsistency; for instance the
...
consequences of functionally inconsistent prediction
maps and the notion of functional inconsistency is
more basic.

\section{Conclusion}
In this letter we have presented
functional inconsistency as an alternative to
...
\acknowledgments
We are thankful to the anonymous referees for their feedback. ASA and Arvind acknowledge
